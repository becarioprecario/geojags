/*
   Copied from file DMNorm.cc from JAGS 4.2.0.
   Copyright by Martin Plummer, 2016.
*/
...
DSARError::DSARError()
  : ArrayDist("dsarerror", 2)
{}


double DSARError::logDensity(double const *x, unsigned int m, PDFType type,
...
void DSARError::randomSample(double *x, unsigned int m,
                          vector<double const *> const &parameters,
                          vector<vector<unsigned int> > const &dims,
...
void DSARError::randomsample(double *x, double const *mu, double const *T,
...
bool DSARError::checkParameterDim(vector<vector<unsigned int> > const &dims) const
{
...
vector<unsigned int> DSARError::dim(vector<vector<unsigned int> > const &dims) const
{
    return dims[0];
}
...
bool
DSARError::checkParameterValue(vector<double const *> const &parameters,
...
void DSARError::support(double *lower, double *upper, unsigned int length,
                     vector<double const *> const &parameters,
                     vector<vector<unsigned int> > const &dims) const
{
...
void DSARError::typicalValue(double *x, unsigned int m,
                          vector<double const *> const &parameters,
                          vector<vector<unsigned int> > const &dims,
                          double const *lower, double const *upper) const
{
...
bool DSARError::isSupportFixed(vector<bool> const &fixmask) const
{
    return true;
}
