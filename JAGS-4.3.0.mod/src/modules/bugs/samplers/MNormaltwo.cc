...
MNormtwoMetropolis::MNormtwoMetropolis(SingletonGraphView const *gv, 
				 unsigned int chain)
...
MNormtwoMetropolis::~MNormtwoMetropolis()
{
    delete [] _mean;
    delete [] _var;
    delete [] _prec;
...
void MNormtwoMetropolis::update(RNG *rng)
{
    ...
    DMNormtwo::randomsample(eps, 0, _var, false, N, rng);
...
void MNormtwoMetropolis::rescale(double p)
{
    ++_n;
    p = min(p, 1.0);
    _sump += p;
...
bool MNormtwoMetropolis::checkAdaptation() const
{
    return (_n_isotonic > 0) && (_meanp >= 0.15) && (_meanp <= 0.35);
}

void MNormtwoMetropolis::getValue(vector<double> &value) const
{
    double const *v = _gv->node()->value(_chain);
    copy(v, v + _gv->length(), value.begin());
}

void MNormtwoMetropolis::setValue(vector<double> const &value)
{
