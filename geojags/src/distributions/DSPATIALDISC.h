#ifndef DSPATIALDISC_H_
#define DSPATIALDISC_H_
 ...
 * x[] ~ dmnorm(mu[], T[,])
 * T[] = tau * (I - rho * W)
 * f(x | mu, T) = sqrt(det(T)) * DISC(-1/2 * (x-mu) %*% T %*% t(x-mu))
 * </pre>
 */
class DSPATIALDISC : public ArrayDist {
public:
  DSPATIALDISC();
