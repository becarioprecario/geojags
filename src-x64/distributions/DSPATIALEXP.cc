...
DSPATIALEXP::DSPATIALEXP()
  : ArrayDist("spatial.exp", 6) //mu, x[], y[], tau, phi, kappa
{}

double DSPATIALEXP::logDensity(double const *x, unsigned int m, PDFType type,
			  ...
			  double const *lower, double const *upper) const
{
    ...
    double const * phi = parameters[4];
    double const * kappa = parameters[5];
    ...
    //VARIANCE matrix
    create_spatialexp(VCOV, xx, yy, m, tau[0], phi[0], kappa[0]);
    ldet = logdet(VCOV, m);
...
void DSPATIALEXP::randomSample(double *x, unsigned int m,
			  vector<double const *> const &parameters,
			  vector<vector<unsigned int> > const &dims,
			  ...
			  RNG *rng) const
{
    ...
    create_spatialexp(Tcopy, xx, yy, m, tau[0], phi[0], kappa[0]);

    randomsample(x, mu, Tcopy, false, m, rng);
}

void DSPATIALEXP::randomsample(double *x, double const *mu, double const *T,
...
bool DSPATIALEXP::checkParameterDim(vector<vector<unsigned int> > const &dims) const
{
...
vector<unsigned int> DSPATIALEXP::dim(vector<vector<unsigned int> > const &dims) const
{
    return dims[0];
}

bool
DSPATIALEXP::checkParameterValue(vector<double const *> const &parameters,
			    vector<vector<unsigned int> > const &dims) const
{
    //unsigned int n = dims[0][0];

    //return check_symmetry(adjacency, n);// && check_symmetric_ispd(precision, n);
    return true; //FIXME: Check parameters
}


void DSPATIALEXP::support(double *lower, double *upper, unsigned int length,
		     vector<double const *> const &parameters,
		     vector<vector<unsigned int> > const &dims) const
{
...
void DSPATIALEXP::typicalValue(double *x, unsigned int m,
			  vector<double const *> const &parameters,
			  vector<vector<unsigned int> > const &dims,
			  double const *lower, double const *upper) const
{
...
bool DSPATIALEXP::isSupportFixed(vector<bool> const &fixmask) const
{
    return true;
}
